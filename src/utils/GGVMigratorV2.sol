...
contract GGVMigratorV2 is IGGVMigrator, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MIN_WETH_DEBT_FOR_MIGRATION = 1 ether;
    uint256 public constant MAX_WETH_DEBT_ERROR = 1 gwei;
    ...
    function calculateSteps(uint256 maxUtilizationD18, uint256 percentageD6)
        public
        view
        returns (uint256 iterations, uint256 wethPerStep, uint256 weethPerStep, uint256 expectedWethAfter)
    {
        uint256 wethDebt = IERC20(WETH_DEBT_TOKEN).balanceOf(address(GGV));
        uint256 wethToMigrate = wethDebt * percentageD6 / 1e6;
        expectedWethAfter = wethDebt - wethToMigrate;

        if (
            wethToMigrate < MIN_WETH_DEBT_FOR_MIGRATION
                || expectedWethAfter > MAX_WETH_DEBT_ERROR && expectedWethAfter < MIN_WETH_DEBT_FOR_MIGRATION
        ) {
        ...
        iterations = Math.ceilDiv(wethToMigrate, maxBorrowWeth);
        if (iterations > MAX_ALLOWED_ITERATIONS) {
            revert TooManyIterations(iterations);
        }

        wethPerStep = wethToMigrate / iterations;
    ...
    function migrate(uint256 maxUtilizationD18, uint256 wethToRepay, uint256 wethToBorrow, uint256 percentageD6)
        external
        onlyOwner
        nonReentrant
    {
        ...
        (uint256 iterations, uint256 wethPerStep, uint256 weethPerStep, uint256 expectedWethAfter) =
        ...
        {
            uint256 ggvWethDebt = IERC20(WETH_DEBT_TOKEN).balanceOf(address(GGV));
            if (
                ggvWethDebt > MAX_WETH_DEBT_ERROR + expectedWethAfter
                    || ggvWethDebt + MAX_WETH_DEBT_ERROR < expectedWethAfter
            ) {
                revert WethDebtTooHigh();
            }

            if (ggvWethDebt < MAX_WETH_DEBT_ERROR) {
                migrated = true;
            }
        }
