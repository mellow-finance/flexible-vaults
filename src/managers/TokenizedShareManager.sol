...
contract TokenizedShareManager is ShareManager, ERC20Upgradeable {
    ...
    function initialize(bytes calldata data) external initializer {
        (address vault_, bytes32 whitelistMerkleRoot_, uint256 sharesLimit_, string memory name_, string memory symbol_)
        = abi.decode(data, (address, bytes32, uint256, string, string));
