...
contract Deploy is Script, Test {
    ...
    function run() external {
        ...
        if (true) {
            _acceptReports(OracleSubmitter(0x34AAc3467099484F67107e448fd089FDafc025D0), deployer);
            return;
        }
        ...
        // queues setup
        // SyncDepositQueue
        vault.createQueue(2, true, proxyAdmin, Constants.USDG, abi.encode(DEFAULT_PENALTY_D6, DEFAULT_MAX_AGE));
        // RedeemQueue
        vault.createQueue(0, false, proxyAdmin, Constants.USDG, new bytes(0));
        ...
        for (uint256 i = 0; i < vault.getAssetCount(); i++) {
            ...
            for (uint256 j = 0; j < vault.getQueueCount(asset); j++) {
                address queue = vault.queueAt(asset, j);
                try ISyncQueue(queue).name() returns (string memory queueName_) {
                    console.log("%s (%s): %s", queueName_, symbol_, queue);
                    continue;
                } catch {}
...

        {
            ...
            for (uint256 i = 0; i < reports.length; i++) {
                ...
                reports[i].priceD18 = uint224(1e30);
        ...
        AcceptanceLibrary.runVaultDeploymentChecks(
            ...
            VaultDeployment({
                ...
                redeemQueueAssets: ArraysLibrary.makeAddressArray(abi.encode(Constants.USDG)),
