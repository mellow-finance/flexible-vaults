...
contract Deploy is Script, Test {
    ...
    address[] assets_ = ArraysLibrary.makeAddressArray(
        abi.encode(Constants.USDT0, Constants.USDE, Constants.SUSDE, Constants.SYRUP_USDT)
    );
    ...
    function run() external {
...

        {
            IRiskManager riskManager = vault.riskManager();
            {
                ...
                console.log("Verifier 0:", verifiers[subvaultIndex]);
    ...
    function _deploySwapModule(address subvault) internal returns (address) {
        ...
        address[] memory actors = ArraysLibrary.makeAddressArray(
            abi.encode(
                curator,
                [Constants.USDT0, Constants.SUSDE, Constants.USDE, Constants.SYRUP_USDT],
                [Constants.USDT0, Constants.SUSDE, Constants.USDE, Constants.SYRUP_USDT],
                _routers()
            )
        );
