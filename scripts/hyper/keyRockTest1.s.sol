...
contract Deploy is DeployAbstractScript {
    function run() external {
        // Deploy a fresh registry + oracle submitter factory + deploy vault factory,
        // or reuse the live Constants.deployVaultFactory constant.
        /*
                OracleSubmitterFactory     0xFAB81ceEB25a76d8736c10686180aa8a115d952C
        ...
        */
        ...
        vault = Vault(payable(address(0x574fFa5Ee3d99e2BCB72aAAbe3D8453D71492883)));
    ...
    /// @dev deposit into a queue; native HYPE is sent as msg.value
    function deposit(address asset, address queue) internal {
        ...
        uint224 amount;
        uint256 value;
        if (asset == Constants.HYPE) {
            symbol = "HYPE";
            value = 1 gwei;
            amount = uint224(value);
        } else {
            symbol = IERC20Metadata(asset).symbol();
            amount = uint224(IERC20(asset).balanceOf(deployer)) / 1000;
    ...
    function setUp() public override {
        ...
        vaultName = "test KeyRock Hyper Loop";
        vaultSymbol = "tKRHyper";
        address allInOne = 0x7538E38335FbeecbF432ba98b55317E10C4C47dc;

        /// @dev fill admin/operational addresses
        proxyAdmin = allInOne;
        lazyVaultAdmin = allInOne;
        activeVaultAdmin = allInOne;
        oracleUpdater = allInOne;
        curator = allInOne;
        feeManagerOwner = allInOne;
        pauser = allInOne;
        ...
        securityParams = IOracle.SecurityParams({
            ...
            redeemInterval: 24 hours // 24h redemption
        });
        ...
        riskManagerLimit = 1e8 ether; // 100M HYPE
        ...
        returns (IDeployVaultFactory.SubvaultParams[] memory subvaultParams)
    {
        ...
        subvaultParams[0].assets = ArraysLibrary.makeAddressArray(abi.encode(Constants.HYPE));
        subvaultParams[0].version = uint256(SubvaultVersion.DEFAULT);
        subvaultParams[0].verifierVersion = 0;
        subvaultParams[0].limit = 1e8 ether; // 100M HYPE
    }
        ...
        returns (IDeployVaultFactory.QueueParams[] memory queues, uint256 queueLimit)
    {
        ...
        */
        ...
        queues[0] = IDeployVaultFactory.QueueParams({
            version: uint256(2),
            isDeposit: true,
            asset: Constants.HYPE,
            data: abi.encode(uint256(0), uint32(365 days)) // penaltyD6, maxAge for SyncDepositQueue
        });

        queues[1] =
            IDeployVaultFactory.QueueParams({version: uint256(0), isDeposit: false, asset: Constants.HYPE, data: ""});
    ...
    {
        allowedAssets = ArraysLibrary.makeAddressArray(abi.encode(Constants.HYPE));
        allowedAssetsPrices = new uint224[](allowedAssets.length);
        allowedAssetsPrices[0] = uint224(1e18); // native HYPE, 18 decimals (1e8 * 10 ** (28 - 18))
    }
